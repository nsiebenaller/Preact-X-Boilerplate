...
import style from './HelpModal.less'

const Actions = (props) => (
  <Button
    class={style['modal-close']}
    onClick={() => props.setState({ helpModalOpen: false })}
  >Close</Button>
)
...
const HelpModal = (props) => (
  <div>
    <HelpOutline
      class="pointer"
      onClick={() => props.setState({ helpModalOpen: true })}
    />
    <Modal
      ...
      content={(() => <Content />)()}
      actions={(() => <Actions setState={props.setState} />)()}
      onClose={() => props.setState({ helpModalOpen: false })}
    />
  </div>
)
